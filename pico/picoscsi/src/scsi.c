...
uint8_t emulationMode = LVDOS_EMULATION;
...
void scsiInitialise(void) {
    ...
    // On a cold-start we always output debug information (ignoring the setting
    // of the debug flags) - as this is useful for initial board testing

    debugPrintf("\r\n\r\nPicoSCSI - Acorn SCSI-1 Emulation\r\n\r\n");
    debugPrintf("(c)2018-2025 Simon Inns\r\n");
    debugPrintf("https://www.domesday86.com\r\n");
    debugPrintf("Open-source GPLv3 firmware\r\n");
    debugPrintf("\r\n");

    // Determine the emulation mode (fixed or LV-DOS)
    // if (hostadapterConnectedToExternalBus()) {
    // 	emulationMode = FIXED_EMULATION;
    // 	debugPrintf("Emulation mode is Winchester (ADFS SCSI-1
    // hard-drive)\r\n"); } else {
    emulationMode = LVDOS_EMULATION;
    debugPrintf("Emulation mode is Philips VP415 (VFS LaserDisc player)\r\n");
    //}

    debugPrintf("\r\n");

    if (debugFlag_scsiState)
        debugPrintf("SCSI State: Initialising SCSI emulation\r\n");
...
void scsiReset(void) {
    ...
    if (debugFlag_scsiState) {
        ...
        emulationMode = LVDOS_EMULATION;
        debugPrintf(
            "Emulation mode is Philips VP415 (VFS LaserDisc player)\r\n");
        // }
...
void scsiProcessEmulation(void) {
    ...
    switch (scsiState) {
        ...
        default:
            if (debugFlag_scsiState)
                debugPrintf("SCSI State: ERROR: Invalid SCSI state!\r\n");
    }

    // Show activity using the status LED on whenever we are not in the bus free
    // state
    if (scsiState == SCSI_BUSFREE)
        statusledActivity(0);
    else
        statusledActivity(1);
}
...
void scsiInformationTransferPhase(uint8_t transferPhase) {
    ...
    switch (transferPhase) {
        ...
        case ITPHASE_MESSAGEOUT:
            ...
            if (debugFlag_scsiState)
                debugPrintf(
                    "SCSI State: Information transfer phase: Message out\r\n");
            break;
...
uint8_t scsiEmulationBusFree(void) {
    ...
    if (debugFlag_scsiState) debugPrintf("SCSI State: Bus Free\r\n");
    ...
    if (debugFlag_scsiState)
        debugPrintf("SCSI State: Selected by host ID %d\r\n", hostIdentifier);
...
uint8_t scsiEmulationCommand(void) {
    ...
    if (debugFlag_scsiState) debugPrintf("SCSI State: Command phase\r\n");
    ...
    switch (commandDataBlock.group) {
        ...
        default:
            if (debugFlag_scsiCommands)
                debugPrintf(
                    "SCSI Commands: ERROR: BAD command group received\r\n");
            break;
    }
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf(
            "SCSI Commands: CDB byte 0 decode: Command group %d (%d bytes) "
            "opcode %d\r\n",
            commandDataBlock.group, commandDataBlock.length,
            commandDataBlock.opCode);
    }

    // Show received byte value
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: Received command operand bytes: %d",
                    commandDataBlock.data[commandDataBlockPointer]);
    }
    ...
    while (commandDataBlockPointer < commandDataBlock.length) {
        ...
        // Show received byte value
        if (debugFlag_scsiCommands)
            debugPrintf(" %d", commandDataBlock.data[commandDataBlockPointer]);

        commandDataBlockPointer++;
    }
    if (debugFlag_scsiCommands) debugPrintf("\r\n");
    ...
    if (debugFlag_scsiCommands)
        debugPrintf(
            "SCSI Commands: ERROR: BAD opcode received - transitioning to bus "
            "free\r\n");
    return SCSI_BUSFREE;
}
...
uint8_t scsiEmulationStatus(void) {
    if (debugFlag_scsiState)
        debugPrintf("SCSI State: Status. Status byte = %d\r\n",
                    commandDataBlock.status);
...
uint8_t scsiEmulationMessage(void) {
    if (debugFlag_scsiState)
        debugPrintf("SCSI State: Message In. Message byte = %d\r\n",
                    commandDataBlock.message);
...
uint8_t scsiCommandTestUnitReady(void) {
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: TESTUNITREADY command (0x00) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
    ...
    if (filesystemReadLunStatus(commandDataBlock.targetLUN)) {
        ...
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Unavailable LUN #%d\r\n",
                        commandDataBlock.targetLUN);
        commandDataBlock.status =
            (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Failed
        commandDataBlock.message = 0x00;
        ...
        requestSenseData[commandDataBlock.targetLUN].errorClass =
            0x00;  // Class 00 error code
        requestSenseData[commandDataBlock.targetLUN].errorCode =
            0x02;  // Unit not ready
...
uint8_t scsiCommandRezeroUnit(void) {
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: REZEROUNIT command (0x01) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }
...
uint8_t scsiCommandRequestSense(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: REQUESTSENSE command (0x03) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }

    // Since this command provides additional error information, it is available
    // even if the LUN is unavailable (no LUN image on file system).

    // Get the requested number of sense bytes
    numberOfSenseBytes = commandDataBlock.data[4];

    // The ACB-4000 manual (section 5.5.1) states that, if the number of sense
    // bytes is less than 4, it should default to four bytes
    ...
    // number would be anything other than 4 (nor any result with less than 4
    // bytes).
    if (debugFlag_scsiCommands)
        debugPrintf("SCSI Commands: Sense bytes = %d\r\n", numberOfSenseBytes);
    ...
    if (requestSenseData[commandDataBlock.targetLUN].errorFlag == false) {
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: No error flagged\r\n");
        ...
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Error flagged - sending to host\r\n");
        if (requestSenseData[commandDataBlock.targetLUN].validAddressFlag ==
            true)
            responseByte = 128;  // set address valid flag
        responseByte +=
            (requestSenseData[commandDataBlock.targetLUN].errorClass & 0x7)
            << 4;  // set error class field
        responseByte +=
            (requestSenseData[commandDataBlock.targetLUN].errorCode &
             0x0F);  // set error code field

        hostadapterWriteByte(responseByte);
        hostadapterWriteByte(
            (uint8_t)((requestSenseData[commandDataBlock.targetLUN]
                           .logicalBlockAddress &
                       0x1F0000) >>
                      16));
        hostadapterWriteByte(
            (uint8_t)((requestSenseData[commandDataBlock.targetLUN]
                       ...
                       0x00FF00) >>
                      8));
        hostadapterWriteByte((uint8_t)((
            requestSenseData[commandDataBlock.targetLUN].logicalBlockAddress &
            0x0000FF)));
    }
...
uint8_t scsiCommandFormat(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: FORMAT command (0x04) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }
    ...
    if (!filesystemReadLunStatus(commandDataBlock.targetLUN)) {
        // If the LUN is unavailable we need to create the LUN image on the file
        // system before formatting it.
        if (!filesystemCreateLunImage(commandDataBlock.targetLUN)) {
            // Could not create LUN image... return with error status
            if (debugFlag_scsiCommands)
                debugPrintf(
                    "SCSI Commands: ERROR: Could not create new LUN image for "
                    "LUN #%d\r\n",
                    commandDataBlock.targetLUN);
            commandDataBlock.status =
                (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
            commandDataBlock.message = 0x00;

            // Set request sense error globals
            requestSenseData[commandDataBlock.targetLUN].errorFlag = true;
            requestSenseData[commandDataBlock.targetLUN].validAddressFlag =
                false;
            requestSenseData[commandDataBlock.targetLUN].errorClass =
                0x02;  // Class 02 error code
            requestSenseData[commandDataBlock.targetLUN].errorCode =
    ...
    dataPattern =
        commandDataBlock.data[2];  // Default fill pattern is 0x6C (108)
    // interleave = (commandDataBlock.data[3] << 8) + commandDataBlock.data[4];

    if (debugFlag_scsiCommands)
        debugPrintf("SCSI Commands: Format option = %d\r\n", formatOptions);

    // Interleave cannot be greater than the number of sectors per track minus
    // one, or we should send a 1A error code (Interleave Error) We don't really
    // care about the interleave... you could uncomment this for a more exact
    // emulation...
    // if (interleave > 32)
    //{
    //// Indicate unsuccessful command in status and message
    // commandDataBlock.status = (commandDataBlock.targetLUN << 5) | 0x02; //
    // 0x02 = Bad commandDataBlock.message = 0x00;
    //
    //// Set request sense error globals
    // requestSenseData[commandDataBlock.targetLUN].errorFlag = true;
    // requestSenseData[commandDataBlock.targetLUN].validAddressFlag = false;
    // requestSenseData[commandDataBlock.targetLUN].errorClass = 0x02; // Class
    // 02 error code requestSenseData[commandDataBlock.targetLUN].errorCode =
    ...
    if (formatOptions == 28 || formatOptions == 30) {
        ...
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Reading the defect list:\r\n");
        ...
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Defect list header:\r\n");
        for (byteCounter = 0; byteCounter < 4; byteCounter++)
            scsiSectorBuffer[byteCounter] = hostadapterReadByte();

        defectListLength = (((uint32_t)scsiSectorBuffer[2] << 8) +
                            (uint32_t)scsiSectorBuffer[3]) /
                           8;
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands:   Length = %d\r\n", defectListLength);

        // Read the defect records
        for (defectListRecords = 0; defectListRecords < defectListLength;
             defectListRecords++) {
            ...
            if (debugFlag_scsiCommands)
                debugPrintf("SCSI Commands: Defect #%d\r\n", defectListRecords);
            if (debugFlag_scsiCommands)
                debugPrintf("SCSI Commands:   Cylinder = %ld\r\n",
                            ((uint32_t)scsiSectorBuffer[0] << 16) +
                                ((uint32_t)scsiSectorBuffer[1] << 8) +
                                (uint32_t)scsiSectorBuffer[2]);
            if (debugFlag_scsiCommands)
                debugPrintf("SCSI Commands:   Head = %d\r\n",
                            scsiSectorBuffer[3]);
            if (debugFlag_scsiCommands)
                debugPrintf("SCSI Commands:   Bytes = %ld\r\n",
                            ((uint32_t)scsiSectorBuffer[4] << 24) +
                                ((uint32_t)scsiSectorBuffer[5] << 16) +
                                ((uint32_t)scsiSectorBuffer[6] << 8) +
                                (uint32_t)scsiSectorBuffer[7]);
        }
    }

    // Create/recreate the LUN data file according to the drive descriptor and
    // fill with the required data pattern byte:
    if (!filesystemFormatLun(commandDataBlock.targetLUN, dataPattern)) {
        // Formatting failed...
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Format failed\r\n");
...
uint8_t scsiCommandRead6(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: READ command (0x08) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d",
                    commandDataBlock.targetLUN);
    }
    ...
    // Make sure the target LUN is started
    if (!filesystemReadLunStatus(commandDataBlock.targetLUN)) {
        // Target LUN is not started.  If the LUN is present, then start it,
        // otherwise return an error.  Note: The original Adaptec SCSI host
        // adapter would always auto-start a LUN if it was present, so we
        // duplicate that behavior here even though it is 'more correct'
        // (according to the specs) to return with error

        // Is the requested LUN available?
        if (debugFlag_scsiCommands)
            debugPrintf(
                "\r\nSCSI Commands: Attempting to Auto-Start LUN (as it is "
                "currently STOPped)\r\n");
        ...
        if (!filesystemSetLunStatus(commandDataBlock.targetLUN, true)) {
            ...
            if (debugFlag_scsiCommands)
                debugPrintf("SCSI Commands: Could not auto-start LUN #%d\r\n",
                            commandDataBlock.targetLUN);
            commandDataBlock.status =
                (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
            commandDataBlock.message = 0x00;
        ...
        if (debugFlag_scsiCommands)
            debugPrintf(
                "SCSI Commands: Requested LUN has been auto-started\r\n");

        // Output the initial debug again (as the command debug information is
        // appended to it)
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Target LUN = %d",
                        commandDataBlock.targetLUN);
    ...
    if (numberOfBlocks == 0)
        numberOfBlocks =
            256;  // 0 = 256 blocks according to the SCSI specification

    // Show the command debug information
    if (debugFlag_scsiCommands)
        debugPrintf(", LBA = %ld, Blocks = %ld\r\n", logicalBlockAddress,
                    numberOfBlocks);
    ...
    if (!filesystemOpenLunForRead(commandDataBlock.targetLUN,
                                  logicalBlockAddress, numberOfBlocks)) {
        // Opening the LUN image failed... try to recover with a little grace...
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: ERROR: Open LUN image failed!\r\n");
        commandDataBlock.status =
            (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
        commandDataBlock.message = 0x00;
    ...
    for (currentBlock = 0; currentBlock < numberOfBlocks; currentBlock++) {
        ...
        if (!filesystemReadNextSector(scsiSectorBuffer)) {
            // Reading from the LUN image failed... try to recover with a little
            // grace...
            if (debugFlag_scsiCommands)
                debugPrintf(
                    "SCSI Commands: ERROR: Could not read next sector from LUN "
                    "image!\r\n");
            commandDataBlock.status =
                (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
            commandDataBlock.message = 0x00;
        ...
        if (hostadapterReadResetFlag()) {
            if (debugFlag_scsiCommands)
                debugPrintf(
                    "SCSI Commands: Read DMA interrupted by host reset at byte "
                    "#%d\r\n",
                    bytesTransferred);
        ...
        if (!debugFlag_scsiBlocks) {
            if (debugFlag_scsiCommands) debugPrintf("%ld ", currentBlock);
        } else {
            if (debugFlag_scsiBlocks) {
                debugPrintf("Hex dump for block #%ld\r\n", currentBlock);
                debugSectorBufferHex(scsiSectorBuffer, 256);
            }
        }
    }
    if (debugFlag_scsiCommands || debugFlag_scsiBlocks) debugPrintf("\r\n");
    ...
    commandDataBlock.message = 0x00;
    if (debugFlag_scsiCommands)
        debugPrintf("SCSI Commands: Read6 command successful\r\n");
...
uint8_t scsiCommandWrite6(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: WRITE command (0x0A) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d",
                    commandDataBlock.targetLUN);
    }
    ...
    if (!filesystemOpenLunForWrite(commandDataBlock.targetLUN,
                                   logicalBlockAddress, numberOfBlocks)) {
        // Opening the LUN image failed... try to recover with a little grace...
        if (debugFlag_scsiCommands)
            debugPrintf(
                "SCSI Command: ERROR: Could not open LUN image for "
                "writing!\r\n");
        commandDataBlock.status =
            (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
        commandDataBlock.message = 0x00;
    ...
    if (debugFlag_scsiCommands)
        debugPrintf(
            "SCSI Commands: Transferring requested blocks from the "
            "host...\r\n");
    for (currentBlock = 0; currentBlock < numberOfBlocks; currentBlock++) {
        ...
        if (hostadapterReadResetFlag()) {
            if (debugFlag_scsiCommands)
                debugPrintf(
                    "SCSI Commands: Write DMA interrupted by host reset at "
                    "byte #%d\r\n",
                    bytesTransferred);
        ...
        if (!filesystemWriteNextSector(scsiSectorBuffer)) {
            // Writing to the LUN image failed... try to recover with a little
            // grace...
            if (debugFlag_scsiCommands)
                debugPrintf(
                    "SCSI Commands: ERROR: Writing to LUN image failed!\r\n");
            commandDataBlock.status =
                (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
            commandDataBlock.message = 0x00;
    ...
    commandDataBlock.message = 0x00;
    if (debugFlag_scsiCommands)
        debugPrintf("SCSI Commands: Write6 command successful\r\n");
...
// doesn't serve a purpose for the file system implementation, but is included
// for completeness.  It simply returns successfully if the specified LUN is
// available.
uint8_t scsiCommandSeek(void) {
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: SEEK command (0x0B) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }
...
uint8_t scsiCommandTranslate(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: TRANSLATE command (0x0F) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }
    ...
    // number LBA is the logical block address HPC is the maximum number of
    // heads per cylinder (reported by disk drive, typically 16 for 28-bit LBA)
    // SPT is the maximum number of sectors per track (reported by disk drive,
    // typically 63 for 28-bit LBA)
    //
    ...
    // division, i.e. the quotient of the division where any fractional part is
    // discarded):
    //
    // C = LBA / (HPC * SPT)
    // H = (LBA / SPT) mod HPC
    ...
    // Note: The 'bytes from index' is the number of bytes from the start of the
    // track (to the defect)
    ...
    if (!filesystemReadLunDescriptor(commandDataBlock.targetLUN,
        ...
        debugPrintf(
            "SCSI Commands: ERROR: Could not read geometry from LUN "
            "descriptor\r\n");
    ...
    bytesFromIndex = ((logicalBlockAddress % 33) + 1) *
                     256;  // Sector number * block size (256)

    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands:   LBA = %ld\r\n", logicalBlockAddress);
        debugPrintf("SCSI Commands:   Heads/Cyl = %ld\r\n", headsPerCylinder);
        debugPrintf("SCSI Commands:   Cylinder = %ld\r\n", cylinderNumber);
        debugPrintf("SCSI Commands:   Head = %ld\r\n", headNumber);
        debugPrintf("SCSI Commands:   Bytes = %ld\r\n", bytesFromIndex);
    }
    ...
    hostadapterWriteByte(
        (uint8_t)((cylinderNumber & 0xFF0000) >> 16));  // Cylinder number MSB
    hostadapterWriteByte(
        (uint8_t)((cylinderNumber & 0xFF00) >> 8));  // Cylinder number
    hostadapterWriteByte(
        (uint8_t)(cylinderNumber & 0xFF));  // Cylinder number LSB

    hostadapterWriteByte((uint8_t)headNumber);  // Head number

    hostadapterWriteByte((uint8_t)((bytesFromIndex & 0xFF000000) >>
                                   24));  // Bytes from index MSB
    hostadapterWriteByte(
        (uint8_t)((bytesFromIndex & 0x00FF0000) >> 16));  // Bytes from index
    hostadapterWriteByte(
        (uint8_t)((bytesFromIndex & 0x0000FF00) >> 8));  // Bytes from index
    hostadapterWriteByte(
        (uint8_t)(bytesFromIndex & 0x000000FF));  // Bytes from index LSB
...
uint8_t scsiCommandModeSelect(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: MODESELECT command (0x15) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }
    ...
    // Make sure the target LUN is started
    if (!filesystemReadLunStatus(commandDataBlock.targetLUN)) {
        // If the target LUN is unavailable then the host is probably attempting
        // to MODESELECT a LUN for which no descriptor exists.  So here we
        // create the LUN descriptor
        if (!filesystemCreateLunDescriptor(commandDataBlock.targetLUN)) {
            // LUN descriptor is unavailable and cannot be created... return
            // with error status
            if (debugFlag_scsiCommands)
                debugPrintf(
                    "SCSI Commands: Create descriptor failed LUN #%d\r\n",
                    commandDataBlock.targetLUN);
            commandDataBlock.status =
                (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
            commandDataBlock.message = 0x00;
        ...
        // LUN files created
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Created descriptor for LUN #%d\r\n",
                        commandDataBlock.targetLUN);
    }

    if (commandDataBlock.data[4] != 22) {
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Bad Argument error\r\n");
        // Indicate unsuccessful command in status and message
        commandDataBlock.status =
            (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
        commandDataBlock.message = 0x00;
    ...
    if (!filesystemWriteLunDescriptor(commandDataBlock.targetLUN,
                                      scsiSectorBuffer)) {
        // Write failed! - Indicate unsuccessful command in status and message
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Writing LUN descriptor failed!\r\n");
        commandDataBlock.status =
            (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Error
        commandDataBlock.message = 0x00;
    ...
    // Indicate successful command in status and message
    if (debugFlag_scsiCommands)
        debugPrintf("SCSI Commands: Writing LUN descriptor successful\r\n");
    commandDataBlock.status = 0x00;  // 0x00 = Good
    commandDataBlock.message = 0x00;
...
uint8_t scsiCommandModeSense(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: MODESENSE command (0x1A) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }

    // We do not check if the LUN is available since there (at this point) may
    // only be a descriptor file for the LUN.  If the descriptor cannot be read
    // we assume that the LUN is completely unavailable

    // We emulate soft-sectored hard drives only, so the drive parameter list
    // must be 22 bytes
    if (debugFlag_scsiCommands)
        debugPrintf("SCSI Commands: List length = %d\r\n",
                    commandDataBlock.data[4]);
    if (commandDataBlock.data[4] != 22) {
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Bad Argument error\r\n");
        // Indicate unsuccessful command in status and message
    ...
    if (filesystemReadLunDescriptor(commandDataBlock.targetLUN,
        ...
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Sending LUN descriptor to host\r\n");
        for (byteCounter = 0; byteCounter < 22; byteCounter++)
            hostadapterWriteByte(scsiSectorBuffer[byteCounter]);
    } else {
        // DSC not OK
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Descriptor read error\r\n");
...
uint8_t scsiCommandStartStop(void) {
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: STARTSTOP command (0x1B) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }
    ...
    if (commandDataBlock.data[4] == 0) {
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Stopping LUN\r\n");

        // Make the target LUN unavailable
        filesystemSetLunStatus(commandDataBlock.targetLUN, false);
    } else {
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: Starting LUN\r\n");

        // Start the LUN
        if (!filesystemSetLunStatus(commandDataBlock.targetLUN, true)) {
            // Could not start LUN... return with error status
            if (debugFlag_scsiCommands)
                debugPrintf("SCSI Commands: Could not start LUN #%d\r\n",
                            commandDataBlock.targetLUN);
            commandDataBlock.status =
                (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
            commandDataBlock.message = 0x00;
...
uint8_t scsiCommandVerify(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf("SCSI Commands: VERIFY command (0x2F) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d",
                    commandDataBlock.targetLUN);
    }
    ...
    if (!filesystemReadLunStatus(commandDataBlock.targetLUN)) {
        // LUN unavailable... return with error status
        if (debugFlag_scsiCommands)
            debugPrintf("\r\nSCSI Commands: Unavailable LUN #%d\r\n",
                        commandDataBlock.targetLUN);
        commandDataBlock.status =
            (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
        commandDataBlock.message = 0x00;
    ...
    // Get the logical block address from the CDB (note: this is different from
    // G0 commands as 4 bytes of LBA are provided)
    logicalBlockAddress = ((uint32_t)commandDataBlock.data[2] << 24) |
    ...
    // Get the requested number of blocks from the CDB
    numberOfBlocks = ((uint32_t)commandDataBlock.data[7] << 8) |
    ...
    // Show the command debug information
    if (debugFlag_scsiCommands)
        debugPrintf(", LBA = %ld, number of blocks = %ld\r\n",
                    logicalBlockAddress, numberOfBlocks);
    ...
    if (filesystemReadLunDescriptor(commandDataBlock.targetLUN,
        ...
        lunSizeInSectors = ((uint32_t)scsiSectorBuffer[15] *
                            (((uint32_t)scsiSectorBuffer[13] << 8) +
                             (uint32_t)scsiSectorBuffer[14])) *
                           33;
    } else {
        // DSC not OK
        if (debugFlag_scsiCommands)
            debugPrintf("SCSI Commands: DSC read error\r\n");
        // Indicate unsuccessful command in status and message
        commandDataBlock.status =
            (commandDataBlock.targetLUN << 5) | 0x02;  // 0x02 = Bad
        commandDataBlock.message = 0x00;
    ...
    if (logicalBlockAddress >= lunSizeInSectors) {
        ...
        if (debugFlag_scsiCommands)
            debugPrintf(
                "SCSI Commands: ERROR: Requested LBA is out-of-range for the "
                "LUN size - Verify failed\r\n");
...
uint8_t scsiWriteFCode(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf(
            "SCSI Commands: WRITE F-Code command (G6 0x0A) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }
    ...
    if (debugFlag_scsiCommands)
        debugPrintf(
            "SCSI Commands: Transferring F-Code buffer from the host...\r\n");
    ...
    if (hostadapterReadResetFlag()) {
        if (debugFlag_scsiCommands)
            debugPrintf(
                "SCSI Commands: Write DMA interrupted by host reset at byte "
                "#%d\r\n",
                bytesTransferred);
        return SCSI_BUSFREE;
    }
    ...
    commandDataBlock.message = 0x00;
    if (debugFlag_scsiCommands)
        debugPrintf("SCSI Commands: Write F-Code command successful\r\n");
...
uint8_t scsiReadFCode(void) {
    ...
    if (debugFlag_scsiCommands) {
        debugPrintf(
            "SCSI Commands: READ F-Code command (G6 0x08) received\r\n");
        debugPrintf("SCSI Commands: Target LUN = %d\r\n",
                    commandDataBlock.targetLUN);
    }
    ...
    if (debugFlag_scsiCommands)
        debugPrintf(
            "SCSI Commands: Transferring F-Code buffer to the host...\r\n");
    ...
    commandDataBlock.message = 0x00;
    if (debugFlag_scsiCommands)
        debugPrintf("SCSI Commands: Read F-Code command successful\r\n");
